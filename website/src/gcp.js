...
export default function GCP(props) {
  ...
  const deployWebsite = () => {
    ...
    axios.post("http://localhost:5000/gcp/sites", payload).then(res => {
      const data = res.data
  ...
  const deleteWebsite = (event) => {
    ...
    axios.post("http://localhost:5000/gcp/sites/"+id, payload).then(res => {
    ...
    websiteData.forEach(item => {
      retUI.push(
        <div className="p-8">
          ...
          onClick={() => {window.open(item.website_url.signed_url, '_blank').focus();}}>Click Me</span>
          <button
            type="button"
            disabled={isLoading}
      ...
      <div className="flex mt-8">
        <div className="pr-8 pl-8">
          <label htmlFor="comment" className="block text-sm font-medium text-gray-700">
            GCP Service Account Creds JSON
          </label>
          <div className="mt-1">
            <textarea
              value={credentials}
